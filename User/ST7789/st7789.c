	...
	while (buff_size > 0) {
		...
		uint16_t* buff16 = (uint16_t*)buff;
			if (DMA_MIN_SIZE <= buff_size)
			{
				...
				{
					buff16[i] = (buff16[i] >> 8) | (buff16[i] << 8);
				}
				HAL_SPI_Transmit_DMA(&ST7789_SPI_PORT, (uint8_t*)buff16, chunk_size);
...
void ST7789_Fill_Color(uint16_t color)
{
	...
	uint16_t* disp_buf16 = (uint16_t*)disp_buf;
		...
		{
			for (uint16_t j = 0; j < sizeof(disp_buf) / 2; j++)
			{
				disp_buf16[j] = color;
			}
